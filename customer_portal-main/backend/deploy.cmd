...
:: 3. Copy built files to wwwroot
IF EXIST "%DEPLOYMENT_SOURCE%\dist" (
  echo Copying dist files to wwwroot...
  xcopy "%DEPLOYMENT_SOURCE%\dist\*" "%DEPLOYMENT_TARGET%" /E /Y /I
  IF !ERRORLEVEL! NEQ 0 goto error
)

:: 4. Copy package.json to wwwroot for dependencies
IF EXIST "%DEPLOYMENT_SOURCE%\package.json" (
  copy "%DEPLOYMENT_SOURCE%\package.json" "%DEPLOYMENT_TARGET%"
  IF !ERRORLEVEL! NEQ 0 goto error
)

:: 5. Copy web.config to wwwroot
IF EXIST "%DEPLOYMENT_SOURCE%\web.config" (
  copy "%DEPLOYMENT_SOURCE%\web.config" "%DEPLOYMENT_TARGET%"
  IF !ERRORLEVEL! NEQ 0 goto error
)

:: 6. Copy error.html to wwwroot
IF EXIST "%DEPLOYMENT_SOURCE%\error.html" (
  copy "%DEPLOYMENT_SOURCE%\error.html" "%DEPLOYMENT_TARGET%"
  IF !ERRORLEVEL! NEQ 0 goto error
)

:: 7. Install production dependencies in wwwroot
